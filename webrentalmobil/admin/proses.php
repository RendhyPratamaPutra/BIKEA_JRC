...
class ClassMobil
{
  ...
  {
    $this->koneksi = mysqli_connect("localhost","root","","jrc");
  }
...
<?php
  ...
  public function reservasi($idmobil,$userid,$kode,$harga,$tglawal,$jum_hari,$status){
  $kali=$harga*$jum_hari;
  $tes=mysqli_query($this->koneksi,"UPDATE reservasi SET total='".$kali."' WHERE user_id='$userid'");
  $query = mysqli_query($this->koneksi,"INSERT INTO reservasi(id_mobil,user_id,nobooking,harga,tgl_pinjam,jum_hari,status,total)
  VALUES ('$idmobil','$userid','$kode','$harga','$tglawal','$jum_hari','$status','$kali')");
  if ($query) {
      echo "
      <script>
