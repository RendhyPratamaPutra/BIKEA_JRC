...
<?php
 ...
 if (isset($_POST["submit"])) {
        ...
        if (aksesadmin($_POST) > 0){
           echo "<script> alert('Selamat Input Berhasil.'); location = 'akses_admin.php'; </script>";
    }else {
      echo "<script> alert('Maaf Data Gagal Disimpan !!'); location = 'akses_admin.php'; </script>";
       }
  }
                          ...
                          <div class="col-lg-6">
                              <div class="card">
                                  ...
                                  <div class="card-body">
                                    ...
                                    echo 'NIP : <select name="nip" onchange="changeValue(this.value)">';  
                                    echo '<option>-------</option>';  
                                    while ($row = mysqli_fetch_array($result)) {  
                                        echo '<option value="' . $row['nip'] . '">' . $row['nip'] . '</option>';  
                                        $jsArray .= "prdName['" . $row['nip'] . "'] = {name:'" . addslashes($row['nama_admin']) . "',desc:'".addslashes($row['kd_grup'])."'};\n";    
                                    }  
                                    echo '</select>';  
                                    ?>  
                                    <?php
                                    $sql = mysqli_query($koneksi,"SELECT * FROM grup ORDER BY kd_grup DESC"); 
                                                      ?>
                                    <br/><br />Nama Siswa : <input type="text" name="nama_admin" id="prd_name"/>
                                     <br><br>
                                     <p>Status:
                                    <select name="kd_grup" id = "prd_desc" >
